/**
 * copyright Wentsing Nee of ThinkSpirit @ 2017 all rights reserved
 */
        ...
        switch (s[i]) {
            case '(':
                ...
                if (std::isdigit(s[i - 1])) {
                    throw std::invalid_argument("a number can not be followed with '(' immediately!");
                }
                st.push(i);
                break;
            case ')':
                ...
                if (std::isdigit(s[i + 1])) {
                    throw std::invalid_argument("')' can not be followed with a number immediately");
                }
